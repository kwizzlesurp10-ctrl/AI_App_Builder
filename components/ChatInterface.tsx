...
const ChatMessage: React.FC<{ message: Message }> = ({ message }) => {
    ...
    return (
        <div className={`flex items-start gap-4 p-4 ${isModel ? 'bg-gray-900/50' : ''}`}>
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ messages, onSendMessage, isLoading, isApiConfigured }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900">
        <div className="flex-1 overflow-y-auto">
            ...
            {isLoading && (
                 <div className="flex items-start gap-4 p-4 bg-gray-900/50">
                    <ModelIcon />
        ...
        <div className="p-4 border-t border-gray-800 bg-gray-900">
            <form onSubmit={handleSubmit} className="flex items-center gap-2">
                ...
                />
                <button type="submit" disabled={isLoading || !input.trim() || !isApiConfigured} className="bg-indigo-600 text-white rounded-lg p-3 h-12 w-12 flex items-center justify-center hover:bg-indigo-500 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors">
